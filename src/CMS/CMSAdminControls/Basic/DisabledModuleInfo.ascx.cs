...
public partial class CMSAdminControls_Basic_DisabledModuleInfo : CMSUserControl
{
    ...
    private bool IsModuleEnabledByAppConfigKeys()
    {
        if (TestAnyKey)
        {
            return TestConfigKeysList.Any(key => ValidationHelper.GetBoolean(SettingsHelper.AppSettings[key], false));
        }

        return TestConfigKeysList.Any() && !TestConfigKeysList.Exists(key => !ValidationHelper.GetBoolean(SettingsHelper.AppSettings[key], false));
    }
