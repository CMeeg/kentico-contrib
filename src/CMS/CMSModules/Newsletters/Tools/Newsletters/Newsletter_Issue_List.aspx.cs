...
public partial class CMSModules_Newsletters_Tools_Newsletters_Newsletter_Issue_List : CMSNewsletterPage
{
    ...
    private string GetTooltipText(string sourceName)
    {
        ...
        switch (sourceName.ToLowerInvariant())
        {
            ...
            case "issueopenedemails":
                tooltipText = GetString("newsletter.openstooltip");
                break;
            ...
            case "issueclickedlinks":
                tooltipText = GetString("newsletter.clickstooltip");
                break;
    ...
    private string GetIssueDisplayName(DataRowView rowView)
    {
        ...
        if (isABTest)
        {
            displayName += UIHelper.GetAccessibleIconTag("NodeLink icon-two-squares-line tn", GetString("unigrid.newsletter_issue.abtesticontooltip"));
        }
    ...
    private int GetClickedLinks(DataRow dataRow)
    {
        ...
        var uniqueClicks = clickedLinks.Sum(row => ValidationHelper.GetInteger(DataHelper.GetDataRowValue(row, "UniqueUserClicks"), 0));
