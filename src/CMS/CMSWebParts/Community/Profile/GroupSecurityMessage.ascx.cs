...
public partial class CMSWebParts_Community_Profile_GroupSecurityMessage : CMSAbstractWebPart
{
    ...
    public string JoinGroupPath
    {
        get
        {
            ...
            if (joinGroupPath != "")
            {
                joinGroupPath = UrlResolver.ResolveUrl(DocumentURLProvider.GetUrl(MacroResolver.ResolveCurrentPath(joinGroupPath)));
            }
    ...
    private static void RedirectToGroupPage(GroupInfo group)
    {
        ...
        {
            URLHelper.Redirect(UrlResolver.ResolveUrl(DocumentURLProvider.GetUrl(GroupInfoProvider.GetGroupProfilePath(group.GroupName, SiteContext.CurrentSiteName))));
        }
    }
